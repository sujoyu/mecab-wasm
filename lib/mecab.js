...
function locateFile(fn) {
  if (fn == "libmecab.data") {
    return new URL("libmecab.data", resourceBase).toString();
  }
  if (fn == "libmecab.wasm") {
    return new URL("libmecab.wasm", resourceBase).toString();
  }
}
...
class Mecab {
  static load(resourceBaseUrl) {
    resourceBase = resourceBaseUrl || import.meta.url;
